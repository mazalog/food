...
export default function CategoriePage({ params }) {
    ...
    const docs = meals.filter(doc => doc.categorie === params.categorie)
        ...
        <>
            <Layout noLogo>
                <div className={styles.container}>
                    {docs.length !== 0 ? <>
                        <Spin meals={docs} spinImage={spinImage ? spinImage.url : false} categoriePage={true} nameCate={spinImage ? '' : params.categorie} />
                        <FoodInfo meals={docs} />
                    </> :
                        <div className={styles.emptydocs}>
                            <h3>Por ahora no tenemos nada para ti en {params.categorie}</h3>
                        </div>}
                </div>
            </Layout>
